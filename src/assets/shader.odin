...
shader_load :: proc(files: ..string) -> (asset: ^Shader, ok: bool) {
	if len(files) != 2 do return nil, false
	filepath := files[0]

	sh1type: u32 = gl.VERTEX_SHADER if strings.ends_with(files[0], ".vert") else gl.FRAGMENT_SHADER
	sh2type: u32 = gl.FRAGMENT_SHADER if strings.ends_with(files[1], ".frag") else gl.VERTEX_SHADER
	if sh1type == sh2type do return nil, false

	success: i32

	sh1 := gl.CreateShader(sh1type); defer gl.DeleteShader(sh1)
	_sh1s, err := os.read_entire_file(files[0], context.temp_allocator); if err != nil && err != os.ERROR_NONE do return nil, false
	...
	_sh2s, err3 := os.read_entire_file(files[1], context.temp_allocator); if err3 != nil && err3 != os.ERROR_NONE do return nil, false
